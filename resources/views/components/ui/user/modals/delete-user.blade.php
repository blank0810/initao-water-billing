<!-- Delete User Modal -->
<div id="deleteUserModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-gray-700">
        <div class="p-6 bg-white dark:bg-gray-900 rounded-t-2xl">
            <div class="text-center">
                ...
                <div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-4 mb-6 text-left">
        ...
        <div class="flex justify-end gap-3 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 rounded-b-2xl">
            <button onclick="closeDeleteUserModal()" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                Cancel
            </button>
            <button id="confirmDeleteBtn" onclick="confirmDeleteUser()" class="px-4 py-2 text-sm font-medium bg-red-600 hover:bg-red-700 text-white rounded-lg transition">
...
async function confirmDeleteUser() {
    ...
    const userName = userToDelete.name || userToDelete.UserName || `User ${userId}`;
    ...
    try {
        ...
        if (result.success) {
            ...
            showToast('Delete User Success', `${userName} has been deleted successfully`, 'success');
            if (window.userManager?.refresh) {
                window.userManager.refresh();
            }
        } else {
            showToast('Delete User Failed', result.message || 'Failed to delete user', 'error');
        }
    } catch (error) {
        console.error('Error deleting user:', error);
        showToast('Delete User Error', 'Network error. Please try again.', 'error');
    } finally {
