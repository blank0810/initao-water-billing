<div x-show="showCreateModal"
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            ...
            <form @submit.prevent="createAccountType()" class="p-6 space-y-4">
                <!-- Account Type Name -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Account Type Name <span class="text-red-500">*</span>
                    </label>
                    <input
                        type="text"
                        x-model="form.at_desc"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                        :class="{'border-red-500': errors.at_desc}"
                        placeholder="e.g., Residential"
                        required
                    />
                    <template x-if="errors.at_desc">
                        <p class="mt-1 text-sm text-red-600" x-text="errors.at_desc[0]"></p>
                    </template>
                </div>
