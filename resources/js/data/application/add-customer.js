...
import { ALL_STATUSES, CHARGE_TYPES } from '../customer/workflow-config.js';
...
function initializeForm() {
    ...
    // Populate barangays from area-data.js
    if (barangaySelect && window.areaData) {
        barangaySelect.innerHTML = '<option value="">Select Barangay</option>';
        window.areaData.barangays.forEach(b => {
            if (b.stat_id === window.areaData.AREA_STATUSES.ACTIVE) {
                const option = document.createElement('option');
                option.value = b.b_desc;
                option.textContent = b.b_desc;
                option.dataset.barangayId = b.b_id;
                barangaySelect.appendChild(option);
            }
        });

        // Auto-populate puroks when barangay changes
        barangaySelect.addEventListener('change', function(e) {
            const selectedOption = e.target.options[e.target.selectedIndex];
            const barangayId = parseInt(selectedOption.dataset.barangayId);
            
            if (purokSelect && barangayId) {
                purokSelect.innerHTML = '<option value="">Select Purok</option>';
                const puroks = window.areaData.getPuroksByBarangay(barangayId);
                puroks.forEach(p => {
                    const option = document.createElement('option');
                    option.value = p.p_desc;
                    option.textContent = p.p_desc;
    ...
    // Populate areas from area-data.js
    if (areaSelect && window.areaData) {
        areaSelect.innerHTML = '<option value="">Select Area</option>';
        window.areaData.areas.forEach(a => {
            if (a.stat_id === window.areaData.AREA_STATUSES.ACTIVE) {
                const option = document.createElement('option');
                option.value = a.a_desc;
                option.textContent = a.a_desc;
                option.dataset.areaId = a.a_id;
                areaSelect.appendChild(option);
            }
        });
    }

    // Area configuration - integrate with meter reader data
    const areaConfig = {
        'Zone A - North District': { meterReader: 'John Smith', readingDay: 5 },
        'Zone B - South District': { meterReader: 'Jane Doe', readingDay: 10 },
        'Zone C - East District': { meterReader: 'Mike Johnson', readingDay: 15 },
        'Zone D - West District': { meterReader: 'Sarah Williams', readingDay: 20 },
